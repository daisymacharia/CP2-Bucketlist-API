...
class BucketlistTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.header['Authorization'] = 'Bearer ' + json.loads(logged_in_user.get_data(as_text=True))['token']
        self.new_bucketlist = {
            'name': 'Learn German'
        }
    ...
    def new_bucket_list(self):
        new_bucket = self.client.post('/api/v1/bucketlists/', headers=self.header,data=json.dumps(self.new_bucketlist))
        return new_bucket
    ...
    def test_create_bucketlist_successfully(self):
        ...
        self.assertEqual(json.loads(new_bucket.get_data(as_text=True))['status'], 201)
    def test_cannot_create_existing_bucketlist(self):
        ...
        self.assertEqual(json.loads(second_bucketlist.get_data(as_text=True))['status'], 409)
    ...
    def test_create_bucketlist_item_successfully(self):
        ...
        self.new_bucket_list()
        response = self.client.post('/api/v1/bucketlists/1/items/',headers=self.header, data=json.dumps(self.new_bucketlist_item))
        self.assertEqual(json.loads(response.get_data(as_text=True))['status'], 201)
    ...
    def test_cannot_create_bucketlist_item_if_unauthorized(self):
        ...
        #add unauthorized user
        response = self.client.post('/api/v1/bucketlists/1/items/',data=json.dumps(self.new_bucketlist_item))
        self.assertEqual(response.status_code, 401)
    ...
    def test_edit_bucketlist(self):
        ...
        response = self.client.put('/api/v1/bucketlists/1',headers=self.header,data=json.dumps(edit_bucketlist))
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_item_successfully(self):
        ...
        self.new_bucket_list()
        response = self.client.post('/api/v1/bucketlists/1/items/',headers=self.header, data=json.dumps(self.new_bucketlist_item))
        response = self.client.delete('/api/v1/bucketlists/1/items/1',headers=self.header)
        self.assertEqual(response.status_code, 200)
    def test_edit_bucketlist_item_successfully(self):
        ...
        self.new_bucket_list()
        response = self.client.post('/api/v1/bucketlists/1/items/',headers=self.header, data=json.dumps(self.new_bucketlist_item))
        response = self.client.put('/api/v1/bucketlists/1/items/1',headers=self.header, data=json.dumps(update_item))
        self.assertEqual(response.status_code, 200)
    ...
    def test_cannot_create_already_existing_item(self):
        ...
        self.new_bucket_list()
        self.client.post('/api/v1/bucketlists/1/items/',headers=self.header, data=json.dumps(self.new_bucketlist_item))
        existing_item = self.client.post('/api/v1/bucketlists/1/items/',headers=self.header, data=json.dumps(self.new_bucketlist_item))
        self.assertEqual(json.loads(existing_item.get_data(as_text=True))['status'], 409)
    def test_cannot_create_item_if_name_not_provided(self):
        """Assert that one cannot create bucketlist item if no items are provided"""
        self.new_bucket_list()
        no_data = {}
        item = self.client.post('/api/v1/bucketlists/1/items/',headers=self.header, data=json.dumps(no_data))
