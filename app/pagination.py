...
class PaginationHelper():
    """Pagination helper class to allow us display a number of items per page
    The __init__ constructor takes in request to allow paginate_query method to
    retrieve the page number value specified with the HTTP request,
    query-QLAlchemy query that the paginate_query method has to paginate,
    resource_for_urlgenerate URLS for the previous and next page
    key_name-paginate_query method will use to return the serialized objects
    schema sub class that marshamallow will use to serialize objects
    """
